...
wchar_t toWideChar(char pixel) {
	...
	if(wideChars.find(pixel) == wideChars.end()) {
		std::cerr << "Character '" << pixel <<"' cannot be written in wide strings" << std::endl;
		pixel = '.';
	}
...
Terminal::Terminal() {
	...
	screenWidth = w.ws_col/2; //due to the usage of wide characters the size is divided by 2
...
void Terminal::DrawFrame() {
	std::wcout << buffer; //print buffer, move cursor to top left and flush
	std::cout  << "\33[1;1H" << std::flush;
}
#else
